        ...
        if ~isempty(esp(idxesp).shank(idxShank).SUA)
            for idxUnit = 1:length(esp(idxesp).shank(idxShank).SUA.cell)
                ...
                for idxOdor = odorsRearranged
                    idxO = idxO + 1;
                    app = [];
                    app = esp(idxesp).shank(idxShank).SUA.cell(idxUnit).odor(idxOdor).AnalogicResponse -...
                    esp(idxesp).shank(idxShank).SUA.cell(idxUnit).odor(idxOdor).AnalogicBaseline;
                    responseCell1Mean(idxCell1, idxO) = mean(app);
                    app3 = abs(esp(idxesp).shank(idxShank).SUA.cell(idxUnit).odor(idxOdor).DigitalResponse)==1; 
                end
                if sum(app3) > 0
                    idxCell2 = idxCell2 + 1;
